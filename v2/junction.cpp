...
bool Junction::insertNaming(int uid, QString name, QString alias, int voltage, QString description, int typeId)
{
...
bool Junction::updateNaming(int uid, QString name, QString alias, int voltage, QString description, int typeId)
{
...
void Junction::on_buttonBox_clicked(QAbstractButton *button){
    ...
    if(button->text() == "Save"){
        if(!uidExist(QString::number(uid))){
            insertNaming(uid, name, alias,ratedVoltage,description,13);
            insertCatalog(uid,uid,junction);
            std::cout<<"insert called"<<std::endl;
        }else{
            updateNaming(uid,name, alias,ratedVoltage,description,13);
