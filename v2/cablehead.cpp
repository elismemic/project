...
bool CableHead::insertCatalog(int id, int typeId)
{
    ...
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "Insert into CatCableHead(Id,TypeId) values(?,?)"));
    db::Params params(pStat->allocParams());
    //bind params
    params <<td_id<<td_typeId;
...
/*bool CableHead::updateCatalog(int id, int typeId)
{
    ...
    db::Transaction trans(pDB);
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "UPDATE CatCableHead set Name = ? where Name = ?"));
    db::Params params(pStat->allocParams());
    //bind params
...
bool CableHead::insertNaming(int uid, QString name, QString alias, int voltage, QString description, int typeId)
{
...
bool CableHead::updateNaming(int uid, QString name, QString alias, int voltage, QString description, int typeId)
{
...
void CableHead::on_buttonBox_clicked(QAbstractButton *button){
    ...
    if(button->text() == "Save"){
        if(!uidExist(QString::number(uid))){
            insertNaming(uid, name, alias,ratedVoltage,description,12);
            insertCatalog(uid,uid);
            std::cout<<"insert called"<<std::endl;
        }else{
            updateNaming(uid,name, alias,ratedVoltage,description,12);
