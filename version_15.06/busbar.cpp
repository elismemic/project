...
bool Busbar::insertCatalog(int id, int typeId)
{
    ...
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "Insert into CatBusbar(Id,TypeId) values(?,?)"));
...
bool Busbar::insertNaming(int uid, QString name, QString alias, int voltage, QString description, int typeId)
{
...
bool Busbar::updateNaming(int uid, QString name, QString alias, int voltage, QString description, int typeId)
{
...
void Busbar::on_buttonBox_clicked(QAbstractButton *button){
    ...
    if(button->text() == "Save"){
        if(!uidExist(QString::number(uid))){
            insertNaming(uid, name, alias,ratedVoltage,description,11);
            insertCatalog(uid,uid);
            std::cout<<"insert called"<<std::endl;
        }else{
            updateNaming(uid,name, alias,ratedVoltage,description,11);
