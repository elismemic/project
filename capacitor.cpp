...
bool Capacitor::insertJobCat(int id)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT,
       "INSERT INTO JobCatalogs(JobID,Type_ID,Cat_ID) VALUES (?,?,?)"));


    //allocate parameters and bind them to the statement
    db::Params params(pStat->allocParams());
    //bind params
    params <<  insert_JobID << insert_TypeID << insert_ID;
    if (!pStat->execute())
    {
        td::String strErr;
        pStat->getErrorStr(strErr);
...
int Capacitor::getFlag()
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_SELECT,
       "Select * FROM JobCatalogs WHERE Type_ID = ? AND Cat_ID = ? ORDER by JobID DESC LIMIT 1"));
...
bool actiCap(int newID,int oldID)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE,
       "Update CatalogCapacitor SET JobID = ? WHERE JobID = ?"));
