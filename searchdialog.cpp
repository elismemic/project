...
bool searchDialog::select_searchConsumer()
{
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT, "SELECT JobID, ID, Name, AliasName, Description, idRatedVoltage, kp0, kq0, kp1, kq1, kp2, kq2, EditFlag from catEnergyConsumer where JobID=? or EditFlag=?"));
...
bool searchDialog::select_searchUnit()
{
    ...
    columns[31].name = "EditFlag";
    columns[31].tdType = td::int4;
    columns[31].len = 32;
...
bool searchDialog::select_searchMotor()
{
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT, "SELECT JobID, ID, Name, AliasName, Description, idNumberOfPhases, idRatedVoltage, nomMechPow, powerFactor, efficiency, loadFactor, rpm, Isu_In, r_x, EditFlag from CatAsynchronousMotor where JobID=? or EditFlag=?"));
...
bool searchDialog::select_searchMotorPla()
{
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT, "SELECT JobID, SystemID, ID, Name, AliasName, Description, idPhaseCode, idCatalogType, idConnectionType,EditFlag from PlacAsynchronousMotor where JobID=? or EditFlag=?"));
...
bool searchDialog::select_searchUnitPla()
{
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT, "SELECT JobID, SystemID, ID, Name, AliasName, Description, idPhaseCode, idCatalogType, idNodeType, idLoadType, idLoadDemandModel, desiredVoltage, grounded, rGround, xGround, EditFlag from PlacGeneratingUnit where JobID=? or EditFlag=?"));
    ...
    columns[15].name = "EditFlag";
    columns[15].tdType = td::int4;
    columns[15].len = 32;
...
bool searchDialog::select_searchShuntRes()
{
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT, "SELECT JobID, SystemID, ID, Name, AliasName, Description, idCatalogType, EditFlag from PlacResistor where JobID=? or EditFlag=?"));
                                   ...
                                   , float kp0, float kq0, float kp1, float kq1, float kp2, float kq2)
{
    ...
    td::INT4 insert_edit(1); // bu kodu yazmışsın... vee sql e EditFlag ı ekleyip  bind params a eklemişsin.
    //diger kataloglarda bu yoktu. ve placementlarda yoktu.. tek tek hepsine ekledim.
                                ...
                                , int idNoP, float nmp, float pow, float eff, float load, int rpm, float isu, float rx)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "INSERT INTO CatAsynchronousMotor(JobID, TypeID, ProducerID, ID, Name, AliasName, Description, idNumberOfPhases, idRatedVoltage, nomMechPow, powerFactor, efficiency, loadFactor, rpm, Isu_In, r_x, EditFlag) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"));
                                   ...
                                   , QString alias, QString desc, int idPhase, int idCatalog, int idConnection)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "INSERT INTO PlacAsynchronousMotor(SystemID, JobID, TypeID, ProducerID, ID, Name, AliasName, Description, idPhaseCode, idCatalogType, idConnectionType, EditFlag) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"));
    ...
    params << insert_systemId << insert_job << insert_type << insert_prod
           << insert_id << refName << refAliasName << refDescription
           << insert_idPhase << insert_idCatalog << insert_idConnection << insert_edit;
                                        ...
                                        , float des, int grounded, float r, float x)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "INSERT INTO PlacGeneratingUnit(SystemID, JobID, TypeID, ProducerID, ID, Name, AliasName, Description, idPhaseCode, idCatalogType, idNodeType, idLoadType, idLoadDemandModel, desiredVoltage, grounded, rGround, xGround, EditFlag) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"));
                                        ...
                                        , QString description, int idCatType)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "INSERT INTO PlacResistor(SystemID, JobID, TypeID, ProducerID, ID, Name, AliasName, Description, idCatalogType, EditFlag) VALUES (?,?,?,?,?,?,?,?,?,?)"));
...
bool searchDialog::update_jobPlacementJobID(int oldJobID, int newJobID)
{
    if (!pDB)
        return false;
...
bool searchDialog::updateConsEditFlag(int jobID, int EditFlag)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE CatEnergyConsumer SET EditFlag=? WHERE JobID=? And ID=?"));

    //allocate parameters and bind them to the statement
    db::Params params(pStat->allocParams());

    //bind params
    params << insert_editFlag << insert_oldJobID << insert_Id;
...
bool searchDialog::updateUnitEditFlag(int jobID, int EditFlag)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE CatGeneratingUnit SET EditFlag=? WHERE JobID=? And ID=?"));
...
bool searchDialog::updateMotorEditFlag(int jobID, int EditFlag)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE CatAsynchronousMotor SET EditFlag=? WHERE JobID=? And ID=?"));
...
bool searchDialog::update_PlaMotorJobIdEditFlag(int oldJobID, int newJobID)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacAsynchronousMotor SET JobID=?, EditFlag=? WHERE JobID=?"));
...
bool searchDialog::updatePlaMotorEditFlag(int jobID, int EditFlag)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacAsynchronousMotor SET EditFlag=? WHERE JobID=? And ID=?"));
...
bool searchDialog::updatePlaMotorEditFlagAfterAct(int oldEditFlag, int newEditFlag)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacAsynchronousMotor SET EditFlag=? WHERE EditFlag=?"));
...
bool searchDialog::update_PlaResistorJobIdEditFlag(int oldJobID, int newJobID)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacResistor SET JobID=?, EditFlag=? WHERE JobID=?"));
...
bool searchDialog::updatePlaResistorEditFlag(int jobID, int EditFlag)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacResistor SET EditFlag=? WHERE JobID=? And ID=?"));
...
bool searchDialog::updatePlaResistorEditFlagAfterAct(int oldEditFlag, int newEditFlag)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacResistor SET EditFlag=? WHERE EditFlag=?"));
...
bool searchDialog::update_PlaUnitJobIdEditFlag(int oldJobID, int newJobID)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacGeneratingUnit SET JobID=?, EditFlag=? WHERE JobID=?"));
...
bool searchDialog::updatePlaUnitEditFlag(int jobID, int EditFlag)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacGeneratingUnit SET EditFlag=? WHERE JobID=? And ID=?"));
...
bool searchDialog::updatePlaUnitEditFlagAfterAct(int oldEditFlag, int newEditFlag)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE PlacGeneratingUnit SET EditFlag=? WHERE EditFlag=?"));
...
void searchDialog::plaMotorClicked(int jobID)
{
    ...
    if(preJob == 0)
    preJob = jobID;
    motorPlac.setSystemID(select_system(), 0, 1);
    motorPlac.setJobID(jobID);
    motorPlac.setProducer(select_producers(), 0, 1);
    motorPlac.setID(0);
    motorPlac.setName("Asynchronous Motor Placement", 1);
    motorPlac.setAliasName("aMotorPla", 1);
    motorPlac.setDescription("This is an Asynchronous Motor Placement", 1);
    motorPlac.setPhaseCode(select_phaseCode(), 0, 1);
    motorPlac.setCatalogType(select_catalogType_Motor(), 0, 1);
    motorPlac.setConnectionType(select_connectionType(), 0, 1);
    ...
    if (motorPlac.exec() == QDialog::Accepted)
    {
        ...
        if(exst == false)
        {
            ...
            //searchJobs nj(this);
            //nj.insert_job(randJ, "Ali", 2, 0, QDate::currentDate(), QDate::currentDate());
            select_searchMotorPla();
        }
...
void searchDialog::plaUnitClicked(int jobID)
{
    ...
    if(preJob == 0)
    preJob = jobID;
    unitPlacement.setSystemID(select_system(), 0, 1);
    unitPlacement.setJobID(jobID);
    unitPlacement.setProducer(select_producers(), 0, 1);
    unitPlacement.setID(0);
    unitPlacement.setName("Generating Unit Placement", 1);
    unitPlacement.setAliasName("gUnitPla", 1);
    unitPlacement.setDescription("This is a Generating Unit Placement", 1);
    unitPlacement.setPhaseCode(select_phaseCode(), 0, 1);
    unitPlacement.setCatalogType(select_catalogType_Unit(), 0, 1);
    unitPlacement.setNodeType(select_nodeType(), 0, 1);
    unitPlacement.setLoadType(select_loadType(), 0, 1);
    unitPlacement.setLoadDemand(select_loadDemand(), 0, 1);
    unitPlacement.setdes(0, 1);
    unitPlacement.setr(0, 1);
    unitPlacement.setx(0, 1);
...
void searchDialog::plaResistorClicked(int jobID)
{
    ...
    if(preJob == 0)
    preJob = jobID;
    shuntResistor.setSystemID(select_system(), 0, 1);
    shuntResistor.setJobID(jobID);
    shuntResistor.setProducer(select_producers(), 0, 1);
    shuntResistor.setID(0);
    shuntResistor.setName("Resistor Placement", 1);
    shuntResistor.setAliasName("resPla", 1);
    shuntResistor.setDescription("This is a Resistor Placement", 1);
    shuntResistor.setCatalogType(select_catalogType_Shunt(), 0, 1);
...
void searchDialog::plaMotorEdtClicked()
{
    setMotorPlaValues();
    plaMotor motorPlac(this);
    int oldJobID = jobVal.toInt();

    if(jobVal.toInt() >=1 && jobVal.toInt() == refJob2)
    {

        motorPlac.setSystemID(select_system(), systemVal.toInt(), 0);
        motorPlac.setJobID(refJob2);
        motorPlac.setProducer(select_producers(), prodVal.toInt(), 0);
        motorPlac.setDisabledID(val2);
        motorPlac.setName(nameVal, 0);
        motorPlac.setAliasName(aliasVal, 0);
        motorPlac.setDescription(descVal, 0);
        motorPlac.setPhaseCode(select_phaseCode(), phaseCVal.toInt(), 0);
        motorPlac.setCatalogType(select_catalogType_Motor(), catTVal.toInt(), 0);
        motorPlac.setConnectionType(select_connectionType(), cTypeVal.toInt(), 0);
    ...
    else if(jobVal.toInt() >=1 && jobVal.toInt() != refJob2)
    {
        ...
        if (motorPlac.exec() == QDialog::Accepted)
            ...
            updatePlaMotorEditFlag(oldJobID, refJob2);
...
void searchDialog::plaUnitEdtClicked()
{
    ...
    if(jobVal.toInt() >=1 && jobVal.toInt() == refJob2)
    {
        ...
        unitPlacement.setSystemID(select_system(), systemVal.toInt(), 0);
        unitPlacement.setJobID(refJob2);
        unitPlacement.setProducer(select_producers(), prodVal.toInt(), 0);
        unitPlacement.setDisabledID(val2);
        unitPlacement.setName(nameVal, 0);
        unitPlacement.setAliasName(aliasVal, 0);
        unitPlacement.setDescription(descVal, 0);
        unitPlacement.setPhaseCode(select_phaseCode(), phaseCVal.toInt(), 0);
        unitPlacement.setCatalogType(select_catalogType_Unit(), catTVal.toInt(), 0);
        unitPlacement.setNodeType(select_nodeType(), nodeTVal.toInt(), 0);
        unitPlacement.setLoadType(select_loadType(), loadTVal.toInt(), 0);
        unitPlacement.setLoadDemand(select_loadDemand(), loadDMVal.toInt(), 0);
        unitPlacement.setdes(dVoltVal.toFloat(), 0);
        unitPlacement.setr(rGroundVal.toFloat(), 0);
        unitPlacement.setx(xGroundVal.toFloat(), 0);
    ...
    else if(jobVal.toInt() >=1 && jobVal.toInt() != refJob2)
    {
        ...
        if (unitPlacement.exec() == QDialog::Accepted)
            ...
            updatePlaUnitEditFlag(oldJobID, refJob2);

            //searchJobs nj(this);
            //nj.insert_job(refJob2, "Ali", 2, 0, QDate::currentDate(), QDate::currentDate());
    ...
    else if(jobVal.toInt() <=0 && jobVal.toInt() == refJob2)
    {
        ...
        unitPlacement.setDescription(descVal, 1);
        unitPlacement.setPhaseCode(select_phaseCode(), phaseCVal.toInt(), 1);
        unitPlacement.setCatalogType(select_catalogType_Motor(), catTVal.toInt(), 1);
        unitPlacement.setNodeType(select_nodeType(), nodeTVal.toInt(), 1);
        unitPlacement.setLoadType(select_loadType(), loadTVal.toInt(), 1);
        unitPlacement.setLoadDemand(select_loadDemand(), loadDMVal.toInt(), 1);
        unitPlacement.setdes(dVoltVal.toFloat(), 1);
    ...
    else if(jobVal.toInt() <= 0 && jobVal.toInt() != refJob2)
    {
        ...
        unitPlacement.setDescription(descVal, 0);
        unitPlacement.setPhaseCode(select_phaseCode(), phaseCVal.toInt(), 0);
        unitPlacement.setCatalogType(select_catalogType_Motor(), catTVal.toInt(), 0);
        unitPlacement.setNodeType(select_nodeType(), nodeTVal.toInt(), 0);
        unitPlacement.setLoadType(select_loadType(), loadTVal.toInt(), 0);
        unitPlacement.setLoadDemand(select_loadDemand(), loadDMVal.toInt(), 0);
        unitPlacement.setdes(dVoltVal.toFloat(), 0);
...
void searchDialog::plaResistorEdtClicked()
{
    ...
    if(jobVal.toInt() >=1 && jobVal.toInt() == refJob2)
    {
        ...
        shuntResistor.setSystemID(select_system(), systemVal.toInt(), 0);
        shuntResistor.setJobID(refJob2);
        shuntResistor.setProducer(select_producers(), prodVal.toInt(), 0);
        shuntResistor.setDisabledID(val2);
        shuntResistor.setName(nameVal, 0);
        shuntResistor.setAliasName(aliasVal, 0);
        shuntResistor.setDescription(descVal, 0);
        shuntResistor.setCatalogType(select_catalogType_Motor(), catTVal.toInt(), 0);
    ...
    else if(jobVal.toInt() >=1 && jobVal.toInt() != refJob2)
    {
        ...
        shuntResistor.setCatalogType(select_catalogType_Motor(), catTVal.toInt(), 1);
        ...
        if (shuntResistor.exec() == QDialog::Accepted)
            ...
            updatePlaResistorEditFlag(oldJobID, refJob2);
