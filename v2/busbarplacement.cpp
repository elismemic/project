...
bool BusbarPlacement::insertNaming(int uid, QString name, QString alias, int cattype, int phasecode, QString description, int type)
{
...
void BusbarPlacement::on_buttonBox_clicked(QAbstractButton *button)
{
    ...
    if(button->text() == "OK"){
        if(!uidExist(QString::number(uid))){
            insertNaming(uid, name, alias,catalogType,phaseCode,description,PlacementType);
            insertPlacement(uid,PlacementType,sectionType);
            QMessageBox::information(this,"Status","Saved successfuly");
        }
        else
            {
            updateNaming(uid, name, alias,catalogType,phaseCode,description,PlacementType);
...
bool BusbarPlacement::updateNaming(int uid, QString name, QString alias, int cattype, int phasecode, QString description, int type)
{
...
bool BusbarPlacement::insertPlacement(int id, int typeId,int sectype)
{
    ...
    db::Transaction trans(pDB);
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "Insert into PlacBusbar(Id,TypeId,JobId, SystemId, SectionType) values(?,?,NULL,0,?)"));
    db::Params params(pStat->allocParams());
    //bind params
    params <<td_id<<td_typeId<<td_sectype;
...
bool BusbarPlacement::deletePlacement(int uid){
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT,"DELETE FROM PlacBusbar where Id = ?"));
