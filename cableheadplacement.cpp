...
void CableHeadPlacement::checkJob() {
    if(globalJobId>0){
...
bool CableHeadPlacement::selectCatType()
{

    int id;
    td::String name;

    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT, "SELECT Id, Name from CatNaming WHERE TypeId = 12"));
...
bool CableHeadPlacement::updateForm(int uid){
    ...
    mem::PointerReleaser<db::IStatement> pStat(pDB->createStatement(db::IStatement::DBS_SELECT,"select Name, Alias_Name, Catalog_Type, Phase_Code, SectionType, Description from  PlacNaming inner join PlacCableHead on PlacNaming.[Id] = PlacCableHead.Id where PlacNaming.Id = ? and PlacNaming.TypeId=?"));
...
bool CableHeadPlacement::updatePlacement(int uid, int sectionType){
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT, "UPDATE PlacCableHead SET SectionType = ? WHERE Id = ? and TypeId = ?"));
...
bool CableHeadPlacement::updateNaming(int uid, QString name, QString alias, int cattype, int phasecode, QString description)
{
...
void CableHeadPlacement::setPlacementType(int PlacementType)
{
...
bool CableHeadPlacement::deleteNaming(int uid){
...
void CableHeadPlacement::setWindowName() {
    if(PlacementType==24)
       this->setWindowTitle("Connectivity Node");
    if(PlacementType==25)
       this->setWindowTitle("Cable Head");
    if(PlacementType==26)
       this->setWindowTitle("Source Sink Connector");
}

bool CableHeadPlacement::updateJobPlacements(int sysId, int jobId, int typeId, int placId){
    if (!pDB)
        return false;
...
bool CableHeadPlacement::insertJobPlacements(int sysId,int jobId,int typeId, int placId){
