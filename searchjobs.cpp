...
bool searchJobs::update_job(int id, QString name, int user, int status, QDate cdate, QDate adate)
{
    if (!pDB)
        return false;
...
bool searchJobs::update_activatedJob(int id, QString name, int user, int status, QDate cdate, QDate adate)
{
    if (!pDB)
        return false;
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE Jobs SET ID=?, Name=?, UserID=?, Status=?, CreationDate=?, ActivationDate=? WHERE ID=?"));

    //allocate parameters and bind them to the statement
    db::Params params(pStat->allocParams());

    //bind params
    params << insert_id << refName << insert_user << insert_stat << insert_cdate << insert_adate << insert_id1;
...
bool searchJobs::update_editFlag(int jobID)
{
    ...
    //create statement using parameters which will be provided later
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_UPDATE, "UPDATE Jobs SET ID=? WHERE ID=?"));

    //allocate parameters and bind them to the statement
    db::Params params(pStat->allocParams());

    //bind params
    params << insert_newID << insert_editO;
...
bool searchJobs::updateJobCatalog(int newJ)
{
    ...
    update_activatedJob(newJ, "Ali", 2, 0, QDate::currentDate(), QDate::currentDate());
...
void searchJobs::on_pushButton_clicked()
{
    ...
    {
        update_job(val.toInt(), aJob.name(),select_ownerID(aJob.ownerName()), aJob.status(),aJob.cdate(),aJob.adate());
        select_jobs();
    }
...
void searchJobs::on_pushButton_4_clicked()//to activate a job
{
    update_status();
    ...
    edit.update_PlaMotorJobIdEditFlag(jobID, max + 1);
    edit.updatePlaMotorEditFlagAfterAct(jobID, max + 1);

    edit.update_PlaResistorJobIdEditFlag(jobID, max + 1);
    edit.updatePlaResistorEditFlagAfterAct(jobID, max + 1);

    edit.update_PlaUnitJobIdEditFlag(jobID, max + 1);
    edit.updatePlaUnitEditFlagAfterAct(jobID, max + 1);



    edit.update_jobCatalogJobID(jobID, max + 1);
    edit.update_jobPlacementJobID(jobID, max + 1);
...
void searchJobs::on_pushButton_2_clicked()
{
    QDialog::accept();
    int inx = index2.row();
    QModelIndex ind = ui->tableView->model()->index(inx,0);
    ui->tableView->selectionModel()->select(index2, QItemSelectionModel::Select);
    val = ui->tableView->model()->data(ind).toString();
    jobID = val.toInt();
    this->close();
}
