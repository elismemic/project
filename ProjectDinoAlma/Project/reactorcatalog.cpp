                                         ...
                                         float reactivePower, float activePowerLoss,float nominalCurrent, QString name)
{
    ...
    db::StatementPtr pStat(pDB->createStatement(db::IStatement::DBS_INSERT,
       "INSERT INTO CatalogReactor(Description,JobID,TypeID,CatNamingID,Alias,ConnectionType,NumberOfPhases,RatedVoltage,MaxVoltage,MinVoltage,NominalSection,ReactivePower,ActivePowerLoss,NominalCurrent,Name) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"));
